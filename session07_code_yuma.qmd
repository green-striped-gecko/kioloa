...
{pdf("./Session 07 Natural Selection/env_rasters.pdf")
...
{pdf(file = "./Session 07 Natural Selection/Mf_PCA.pdf", height = 6, width = 6)
...
```{r, include = FALSE}
{png(file = "./Session 07 Natural Selection/Mf_PCA.png", units = 'in', height = 6, width = 6, res = 200)
  pcaplot <- plot(pc, choices = c(1, 2), type = "n", xlab=x.lab, ylab=y.lab, cex.lab=1)
  with(env.site, points(pc, display = "sites", col = env.site$cols, pch = env.site$pch, cex=1.5, bg = env.site$cols))
  legend("topleft", legend = env.site$site, col=env.site$cols, pch=env.site$pch, pt.cex=1, cex=0.50, xpd=1, box.lty = 0, bg= "transparent")
  dev.off()
...
![](Session%2007%20Natural%20Selection/Mf_PCA.png) \## 2. Variable filtering
...
```{r, eval=FALSE}
#plot RDA1, RDA2
{pdf(file = "./Session 07 Natural Selection/Mf_RDA.pdf", height = 6, width = 6)
...
```{r, include=FALSE}
#plot RDA1, RDA2
{png(file = "./Session 07 Natural Selection/Mf_RDA.png", units = 'in', res = 200, height = 6, width = 6)
pRDAplot <- plot(Mf.RDA, choices = c(1, 2), type="n", cex.lab=1, xlab=x.lab, ylab=y.lab)
with(env.site, points(Mf.RDA, display = "sites", col = env.site$cols, pch = env.site$pch, cex=1.5, bg = env.site$cols))
text(Mf.RDA, "bp",choices = c(1, 2), labels = c("WC_bio5", "WC_bio15", "WC_bio19"), col="blue", cex=0.6)
legend("topleft", legend = env.site$site, col=env.site$cols, pch=env.site$pch, pt.cex=1, cex=0.50, xpd=1, box.lty = 0, bg= "transparent")
...
![](Session%2007%20Natural%20Selection/Mf_RDA.png)
...
#plot RDA1, RDA2
{pdf(file = "./Session 07 Natural Selection/Mfcandidate_RDA.pdf", height = 6, width = 6)
...
#plot RDA1, RDA2
{png(file = "./Session 07 Natural Selection/Mfcandidate_RDA.png", units = 'in', res = 200, height = 6, width = 6)
...
![](Session%2007%20Natural%20Selection/Mfcandidate_RDA.png)
...
The input is a structure file with genotypes codified as counts of the major alelle per individual per locus (0 hom minor allele (mA), 1 het, 2 hom major allele (MA)), with sample's phenotype or trait in the first column instead of the population.

::: callout-note
If the trait or phenotype is categorical instead of continous, it should be tranformet into a factor.
:::
...
table(CaRF$Size)
set.seed(123)
ind <- sample(2, nrow(CaRF), replace = TRUE, prob = c(0.7, 0.3))
train <- CaRF[ind==1,]
test <- CaRF[ind==2,]
train$Size
...
###Expected output \# OOB estimate of error rate: 27.69% \# Confusion matrix: \# 1 2 class.error \# 1 93 34 0.2677165 \# 2 38 95 0.2857143 \###

> 70% accurary is no too bad, but can we improive it
...
Here we have also Kappa, a mesure of how much better the model clasitication is than a random clasification. This is importa when when there is a significant skew in sample sizes.
...
###The next couple of steps could take too long, so here are the commands for you to try in your own time control \<- trainControl(method="repeatedcv", number=10, repeats=3, search="random") set.seed(123) mtry \<- sqrt(ncol(CaRF) / 3) rf_random \<- train(Size\~., data=CaRF, method="rf", metric=metric, tuneLength=15, trControl=control) print(rf_random) plot(rf_random)

####Expected output #No pre-processing #Resampling: Cross-Validated (10 fold, repeated 3 times) #Summary of sample sizes: 327, 327, 327, 326, 327, 327, ... #Resampling results across tuning parameters:

# mtry Accuracy Kappa

#195 0.7703453 0.5403670 #348 0.7685185 0.5367943 #526 0.7684685 0.5367281 #665 0.7721972 0.5441283 #953 0.7730731 0.5458833 #1011 0.7694444 0.5386467 #1017 0.7702202 0.5401206 #1038 0.7685435 0.5367490 #1115 0.7740240 0.5477867 #1142 0.7750000 0.5497308 #1253 0.7740490 0.5478857 #1268 0.7740240 0.5477339 #1627 0.7768018 0.5533787 #1842 0.7768268 0.5533826 #2013 0.7749750 0.5496527

#Accuracy was used to select the optimal model using the largest value. #The final value used for the model was mtry = 1842. \####
...
control \<- trainControl(method="repeatedcv", number=5, repeats=3, search="grid") tunegrid \<- expand.grid(.mtry=(1650:1750)) modellist \<- list() seed\<-123 metric \<- "Accuracy" for (ntree in c(200, 300, 500, 1000)) { set.seed(seed) fit \<- train(Size\~., data=CaRF, method="rf", metric=metric, tuneGrid=tunegrid, trControl=control, ntree=ntree) key \<- toString(ntree) modellist\[\[key\]\] \<- fit } \# Then you can compare tuning results results \<- resamples(modellist) summary(results) dotplot(results)
...
But the best cobination was ntree = 200 and mtry= 1680

Now We run our forest with the optimal set up based on the tuning

```{r}

rf <- randomForest(Size~., data=train,
                   ntree = 200,       # number of trees
                   mtry = 1680,          # number of variables tried at each split
                   importance = TRUE,
                   proximity = TRUE)
print(rf)
...
###Expected output #OOB estimate of error rate: 23.48% #Confusion matrix: \# 1 2 class.error #1 93 34 0.2677165 #2 26 107 0.1954887 \###

Around a 5% accuracy gain, looks like not much, but a disclaimer, these are just SNPs from the 4 chomosomes we already identified as important for growth.
...
Ok let's change approach and check concodance:

##### RAINBOWR

First, we integrte our inputs in to a GWAS format for RAINBOWR.
...
modify.CaGW.res <- modify.data(pheno.mat = CaGW_phe, geno.mat = CaGW_geno_T, map = CaGW_map, return.ZETA = TRUE, return.GWAS.format = TRUE)
...
Now we calcualte ZETA, a list of genomic relationship matrix (GRM). ZETA is basically a random effect and accounts for possible confounding/covariant factors like population structure and relatedness.
...
## Other methods to control for population structure cofactors

## structure.matrix, n.PC should be \> 0
...
We can see, first the QQ plot, it shows that our data has a little diviaiton from the expected distribution, but this is just at the end of the tail.

Second, the manhattan plot shows few SNPs that are signifiantly associated to size, but more importantly, it shows a peak of relatively high-importance SNPs at the end of chromosome 16.

So, what are the most importnat important SNPs?
...
To determine that, we use a mixed model approach to calculate heritabilty, in other words the genetic contribution to our trait variation

```{r}

K.A2<-calcGRM(genoMat = CaGW_geno_T)
k.AA2 <- K.A2 * K.A2
...
EM3.res <- EM3.cpp(y = pheno.mat2, X = NULL, ZETA = ZETA2)
...
R2 <- cor(x = pheno.mat[, 1], y = yPred) ^ 2
