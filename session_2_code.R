...
#this dataset is to assess population structure and diversity in Uperoleia crassa, 
#from https://onlinelibrary.wiley.com/doi/epdf/10.1111/mec.16597

#gl <- dartR::gl.read.dart("Report_DUp20-4995_1_moreOrders_SNP_mapping_2.csv", ind.metafile = "Uperoleia_metadata.csv")

load('./data/session_2.RData') # data named gl

####################Luis/emily I would change this to just load the gl as it takes a while to load


#These are monsoonal tropical frogs, who are very common and abundant.
#They do interesting and weird stuff evolutionarily and reproductively
...
########Clean up your dataset to remove the most egregiously bad loci and individuals #########
########this set of filtering can be used across analyses
...
#########Filter data for population structure and phylogenetic analyses ############

#loci on the same fragment and missing data are not well supported in 
#these analyses. 

#Structure-like analyses dislike missing data
#we are going to filter even harder than normal here just so that we can see a result
...
##you can write this file out for various analyses that we will not go
##into here

#gl2structure(XXXX)
#gl2faststructure(XXXX)

#test it out
#this is a structure-like analysis called SNMF
#this code is not all you would need to do to publish this result
#its just a to show you the result

## RUN sNMF (LEA) ##

# LEA requires the genotype style file
gl2faststructure(gl.structure, outfile = "gl_structure.fstr", outpath = './data/')
struct2geno("./data/gl_structure.fstr", ploidy = 2, FORMAT = 2) ###this hates any loci with all heterozygotes

snmf.Sy.K1_8.10 = snmf("./data/gl_structure.fstr.geno", K = 1:8, entropy = T, ploidy = 2, project="new", repetitions = 10)  
...
#have a go at altering various parameters and seeing how this changes
#your answers. One thing that regularly changes with MAF filters 
#is the amount of variance explained. Removing more minor alleles increases
#the variance explained in a PCoA. Think about why that would be the case.


#########Filtering for Tajima's D #######################

#Now we are going to look at how filtering can affect your understanding of 
#demographic processes that population is 

#Significant negative values of Tajima's D are due to an excess of rare alleles 
#and are consistent with range expansion. Significant positive values are 
#associated with population contraction.

#Significance (a P-value) cannot be calculated without a simulation. Please ask
# me how to do this using Hudson's ms program. 

#Rare alleles matter! Lets look at how singletons impact our 
#estimations of whether a population is expanding or contracting


#This is a function written to calculate Tajima's D from SNP data.
#It will create the function in your global environment so that you can call it.
...
# 1. Read depth filtering
# 2. Minor allele filtering
# 3. Read depth

#We will start with our lightly cleaned data and filter this for the tajima's 
#calculation
...
####Now lets try it where we don't remove singletons or secondaries
##### AND  we filter for read depth to remove singletons where we are 
##### not confident about their base calls
...
# For our Kimberley population, removing minor alleles give the actual wrong 
# answer and suggests the population is contracting. This would have 
# contradicted the rest of the findings of the paper, and been an artefact
# of the filtering process.

# This shows how understanding the population genetic theory for a metric is 
# crucial to useful analyses.
