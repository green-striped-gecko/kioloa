...
If the trait or phenotype is categorical instead of continuous, it should be tranformed into a factor.
:::
...
rf_default <- train(Size~., data=CaRF, method="rf", metric=metric, 
                    tuneGrid=tunegrid, trControl=control) # ~ 3mins
print(rf_default)
# plot(rf_default) Error: There are no tuning parameters with more than 1 value.
...
set.seed(123) 
mtry <- sqrt(ncol(CaRF) / 3) 
rf_random <- train(Size~., data=CaRF, method="rf", metric=metric, 
                   tuneLength=15, trControl=control) print(rf_random)
plot(rf_random)
```
...
#Summary of sample sizes: 327, 327, 327, 326, 327, 327, ... 
...
tunegrid <- expand.grid(.mtry=(1650:1750)) 
modellist <- list() 
seed <-123 
metric <- "Accuracy" 
for (ntree in c(200, 300, 500, 1000)) { 
  set.seed(seed) 
  fit <- train(Size~., data=CaRF, method="rf",
               metric=metric, tuneGrid=tunegrid, trControl=control, ntree=ntree) 
  key <- toString(ntree) 
  modellist[[key]] <- fit } # Then you can compare tuning results 
...
###Expected output
#OOB estimate of error rate: 23.48% #Confusion matrix: 
#    1  2  class.error 
# 1 93 34  0.2677165 
# 2 26 107 0.1954887 
```
...
Now just check what are these markers
...
modify.CaGW.res <- modify.data(pheno.mat = CaGW_phe, 
                               geno.mat = CaGW_geno,
                               map = CaGW_map, return.ZETA = TRUE,
...
normal.res <- RGWAS.normal(pheno = pheno.GWAS, 
                           geno = geno.GWAS, ZETA = ZETA, 
                           structure.matrix= QMATRIX, n.PC = 4, P3D = TRUE)
...
normal.res <- RGWAS.normal(pheno = pheno.GWAS, geno = geno.GWAS, 
                           ZETA = ZETA, structure.matrix= QMATRIX, 
                           covariate = SEX_AGE, n.PC = 4, P3D = TRUE)
...
![Q-Q plot](images/session7_qqplot.png)

![Manhattan plot](images/session7_manhattan.png)
...
To determine that, we use a mixed model approach to calculate heritable, in other words the genetic contribution to our trait variation

```{r}

K.A2<-calcGRM(genoMat = CaGW_geno)
k.AA2 <- K.A2 * K.A2
