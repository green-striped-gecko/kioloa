...
If you do not already have it, **download** the `utils.R` file:
...
library(downloadthis)
  ...
  download_file(path = './utils.R',
    output_name = "utils",
    output_extension = ".R",
    button_label = "Download utils.R",
    button_type = "default",
    has_icon = TRUE,
    icon = "fa fa-file-csv"
  )
...
unlink(c('dat.geno', 'dat.lfmm', 'dat.snmfProject', 'dat.snmf', 'dat.lfmm_imputed.lfmm'), recursive = T)
```
...
```{r, include = FALSE}
{png(file = "./images/Mf_PCA.png", units = 'in', height = 6, width = 6, res = 200)
  pcaplot <- plot(pc, choices = c(1, 2), type = "n", xlab=x.lab, ylab=y.lab, cex.lab=1)
  with(env.site, points(pc, display = "sites", col = env.site$cols, pch = env.site$pch, cex=1.5, bg = env.site$cols))
  legend("topleft", legend = env.site$site, col=env.site$cols, pch=env.site$pch, pt.cex=1, cex=0.50, xpd=1, box.lty = 0, bg= "transparent")
  dev.off()
...
![](images/Mf_PCA.png) \## 2. Variable filtering
...
```{r, include=FALSE}
#plot RDA1, RDA2
{png(file = "./images/Mf_RDA.png", units = 'in', res = 200, height = 6, width = 6)
pRDAplot <- plot(Mf.RDA, choices = c(1, 2), type="n", cex.lab=1, xlab=x.lab, ylab=y.lab)
with(env.site, points(Mf.RDA, display = "sites", col = env.site$cols, pch = env.site$pch, cex=1.5, bg = env.site$cols))
text(Mf.RDA, "bp",choices = c(1, 2), labels = c("WC_bio5", "WC_bio15", "WC_bio19"), col="blue", cex=0.6)
legend("topleft", legend = env.site$site, col=env.site$cols, pch=env.site$pch, pt.cex=1, cex=0.50, xpd=1, box.lty = 0, bg= "transparent")
...
#plot RDA1, RDA2
{png(file = "./images/Mfcandidate_RDA.png", units = 'in', res = 200, height = 6, width = 6)
...
unlink('individual_env_data.csv')

```

![](images/Mfcandidate_RDA.png)
